            ...
            const startFrame =
                index === 0
                    ...
                    : clips
                          .slice(0, index)
                          .reduce(
                              (sum, c) => sum + (c.durationInFrames || 0),
                              0
                          );
    ...
    const addClipToTimeline = useCallback(
        (clip: Clip, trackId?: string) => {
            ...
            setTracks((prev) => {
                ...
                const clipTypeForMatching =
                    clip.type === 'image' ? 'video' : clip.type;
                const matchingTrack = prev.find(
                    (t) => t.type === clipTypeForMatching
                );
    ...
    const handleDragOver = useCallback(
        (event: DragOverEvent) => {
            ...
            if (!targetTrack) {
                ...
                const clipTrack = tracks.find((t) =>
                    t.clips.some((c) => c.id === overId)
                );
                if (clipTrack) {
    ...
    const handleDragEnd = useCallback(
        (event: DragEndEvent) => {
            ...
            if (activeId.startsWith('library-asset-')) {
                ...
                const assetId = activeId.replace('library-asset-', '');
                const asset =
                    assetFromData || assets.find((a) => a.id === assetId);
            ...
            if (sourceTrackForReorder) {
                ...
                const targetTrackForReorder = tracks.find(
                    (t) => t.id === overId
                );
                if (
            ...
            if (targetClipInSameTrack) {
                ...
                const oldIndex = sourceTrack.clips.findIndex(
                    (c) => c.id === activeId
                );
                const newIndex = sourceTrack.clips.findIndex(
                    (c) => c.id === overId
                );
            ...
            if (targetTrack) {
                ...
                if (targetTrack.type === clipTypeForMatching) {
                    ...
                    const clipsToConsider = targetTrack.clips.filter(
                        (c) => c.id !== activeId
                    );
                    let dropFrame = 0;
                    if (clipsToConsider.length > 0) {
                        ...
                        const maxEndFrame = Math.max(
                            ...clipsToConsider.map(
                                (c) => (c.startFrame || 0) + c.durationInFrames
                            )
                        );
                    ...
                    setTracks((prevTracks) => {
                        ...
                        if (targetTrackIndex !== -1) {
                            ...
                            newTracks[targetTrackIndex].clips = newTracks[
                                targetTrackIndex
                            ].clips.filter((c) => c.id !== activeId);
            ...
            const targetClipInOtherTrack = tracks.find((t) =>
                t.clips.some((c) => c.id === overId)
            );

            if (
                targetClipInOtherTrack &&
                targetClipInOtherTrack.id !== sourceTrack.id
            ) {
                const targetClip = targetClipInOtherTrack.clips.find(
                    (c) => c.id === overId
                );
                if (targetClip) {
                    // Check if clip type is compatible with target track type
        ...
        (clipId: string) => {
            setTracks((prevTracks) => {
                return prevTracks.map((track) => {
                    ...
                    const clipToDelete = track.clips.find(
                        (c) => c.id === clipId
                    );
                    if (!clipToDelete) {
                    ...
                    const sortedClips = [...updatedClips].sort(
                        (a, b) => (a.startFrame || 0) - (b.startFrame || 0)
                    );

                    // Recalculate startFrames for remaining clips to fill the gap
                    const recalculatedClips =
                        recalculateStartFrames(sortedClips);
                        ...
                        <div
                            ...
                            <span
                                ...
                                {activeDragTrack.type === 'video'
                                    ? `Image/${
                                          activeDragTrack.name ||
                                          `Track ${activeDragTrack.id.slice(
                                              0,
                                              8
                                          )}`
                                      }`
                                    : activeDragTrack.name ||
                                      `Track ${activeDragTrack.id.slice(0, 8)}`}
                            </span>
                            ...
                            <span
                                ...
                                {activeDragTrack.clips.length} clip
                                {activeDragTrack.clips.length !== 1 ? 's' : ''}
                            </span>
                        </div>
                    </DragOverlay>
                )}
